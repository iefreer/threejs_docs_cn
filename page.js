var onDocumentLoad = function ( event ) {
    ...
    if(pathname !== "/threejs/docs/default.html") {
	...
	text = text.replace( /\[page:([\w\u4e00-\u9fa5\(\)\.]+)\]/gi, "[page:$1 $1]" ); // [page:name] to [page:name title]
	text = text.replace( /\[page:\.([\w\u4e00-\u9fa5\(\)\.]+) ([\w\u4e00-\u9fa5\(\)\.\s]+)\]/gi, "[page:" + name + ".$1 $2]" ); // [page:.member title] to [page:name.member title]
	text = text.replace( /\[page:([\w\u4e00-\u9fa5\(\)\.]+) ([\w\u4e00-\u9fa5\(\)\.\s]+)\]/gi, "<a href=\"javascript:window.parent.goTo('$1')\" title=\"$1\">$2</a>" ); // [page:name title]
